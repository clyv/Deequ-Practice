...
The pipeline built in `notebooks/project1.ipynb` runs on Google Colab against ~10M rows of monthly NYC taxi data. The core PyDeequ validation logic is **identical** in production — only the compute environment and data I/O change.
...
On a single Colab node, this pipeline processes ~10M rows in a few minutes.  
...
That's the core value of building on PySpark and PyDeequ: the code you write at 10M rows is production-ready at 3B rows without rewriting anything.
